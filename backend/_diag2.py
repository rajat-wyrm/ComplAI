...
def ok(label, detail=""):
    print("  PASS  " + label + (" | " + detail[:60] if detail else ""))
    results.append(("PASS", label, ""))

def fail(label, detail=""):
    print("  FAIL  " + label + (" | " + detail[:60] if detail else ""))
    results.append(("FAIL", label, detail))

def chk(label, fn):
    try:
        r = fn(); ok(label, str(r)[:55] if r else ""); return True
    except Exception as e:
        fail(label, str(e)[:80]); return False

async def achk(label, coro):
    try:
        r = await coro; ok(label, str(r)[:55] if r else ""); return True
    except Exception as e:
        fail(label, str(e)[:80]); return False

async def main():
    print("\n" + SEP)
    print("  AI Compliance Copilot - System Diagnostic")
    print(SEP + "\n")

    print("[ENV]")
    for k in ["DATABASE_URL","REDIS_URL","DEEPSEEK_API_KEY","SECRET_KEY","ENVIRONMENT"]:
        v = os.environ.get(k, "")
    ...
    for f in sorted(base.rglob("app/**/*.py")):
        ...
        if r.returncode != 0:
            ...
            so += 1
    print("  Syntax: " + str(so) + " OK, " + str(sf) + " errors")

    print("\n[HTTP - server must be running]")
    import urllib.request
    for url in ["http://localhost:8000/","http://localhost:8000/api/health",
                "http://localhost:8000/docs"]:
        try:
            with urllib.request.urlopen(url, timeout=3) as resp:
                print("  HTTP " + str(resp.status) + "  " + url)
        except Exception as e:
            print("  OFFLINE  " + url + " - " + str(e)[:40])
    ...
    if failed:
        ...
        for x in failed:
            ...
            print("    FAIL  " + x[1] + (" | " + detail[:55] if detail else ""))
    else:
        print("  ALL CHECKS PASSED - System fully healthy!")
    print("")
    print("  Admin: admin@compliance.ai / Admin@123456")
    print("  Docs:  http://localhost:8000/docs")
    print("  App:   http://localhost:3001")
    print(SEP + "\n")
